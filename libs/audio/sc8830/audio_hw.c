...
static int out_set_parameters(struct audio_stream *stream, const char *kvpairs)
{
    ...
    if (ret >= 0) {
        ...
        pthread_mutex_lock(&adev->lock);
        pthread_mutex_lock(&out->lock);
		  if (((adev->out_devices & AUDIO_DEVICE_OUT_ALL_EX) != val) && ((val != 0)) //val=0 will cause XRUN. So ignore the "val=0"expect for closing FM path.
...
static void audiodebug_process(struct tiny_audio_device *adev,struct str_parms *parms)
{
    ...
    if (ret >= 0) {
        pthread_mutex_lock(&adev->lock);
        val = atoi(value);
        ALOGI("adev_set_parameters test_stream_route: get adev lock adev->devices is %x,%x",adev->out_devices,adev->out_devices&AUDIO_DEVICE_OUT_ALL_EX);
        if(((val & (AUDIO_DEVICE_OUT_WIRED_HEADSET | AUDIO_DEVICE_OUT_WIRED_HEADPHONE | AUDIO_DEVICE_OUT_SPEAKER | AUDIO_DEVICE_OUT_EARPIECE | AUDIO_DEVICE_OUT_BLUETOOTH_A2DP |
            AUDIO_DEVICE_OUT_BLUETOOTH_SCO | AUDIO_DEVICE_OUT_BLUETOOTH_SCO_HEADSET | AUDIO_DEVICE_OUT_BLUETOOTH_SCO_CARKIT)) != 0) && ((adev->out_devices & AUDIO_DEVICE_OUT_ALL_EX) != val)){
            adev->out_devices &= ~AUDIO_DEVICE_OUT_ALL_EX;
            adev->out_devices |= val;
            if(adev->active_output)  {
                pthread_mutex_lock(&adev->active_output->lock);
                adev->active_output->devices &= ~AUDIO_DEVICE_OUT_ALL_EX;
